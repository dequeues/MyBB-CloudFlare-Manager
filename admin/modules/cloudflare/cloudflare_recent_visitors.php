...
$page->add_breadcrumb_item("Recent Visitors", "index.php?module=cloudflare-recent_visitors");
...
if(!$mybb->input['action'] && !$mybb->input['onlyshow'])
{
	...
	$page->output_nav_tabs($sub_tabs, 'recent_24');
	...
	$array = objectToArray($cloudflare->fetch_recent_visitors("", "24")->response);
	...
	$profile_page = "index.php?module=cloudflare-recent_visitors";
	...
	$table->output("Recent Visitors Data - 24 Hours (" . my_number_format($count) . " Total)");
	...
	$form = new Form("index.php?module=cloudflare-recent_visitors", "post");
...
elseif(!$mybb->input['action'] && $mybb->input['onlyshow'] == "regular")
{
	...
	$array = objectToArray($cloudflare->fetch_recent_visitors("r", "24")->response);
	...
	$profile_page = "index.php?module=cloudflare-recent_visitors&amp;onlyshow=regular";
...
elseif(!$mybb->input['action'] && $mybb->input['onlyshow'] == "bot")
{
	...
	$array = objectToArray($cloudflare->fetch_recent_visitors("s", "24")->response);
	...
	$profile_page = "index.php?module=cloudflare-recent_visitors&amp;onlyshow=bot";
...
elseif(!$mybb->input['action'] && $mybb->input['onlyshow'] == "threat")
{
	...
	$profile_page = "index.php?module=cloudflare-recent_visitors&amp;onlyshow=threat";
	...
	echo multipage($quantity, (int)$perpage, (int)$page, $profile_page);
   	...
   	foreach(array_slice($array['ips'], $start, $perpage) as $n => $data)
   	{
		...
		$table->construct_cell("<strong>" . ++$number . "</strong>", array('width' => '1%'));
		$table->construct_cell("<a href=\"index.php?module=cloudflare-whois&amp;action=lookup&amp;server=" . $data['ip'] . "\" target=\"_blank\">" . $data['ip'] . "</a> " . $isyou, array('width' => '25%'));
		$table->construct_cell($data['classification'], array('width' => '25%'));
		$table->construct_cell(my_number_format($data['hits']), array('width' => '25%'));
...
elseif(!$mybb->input['action'] && $mybb->input['onlyshow'] == "all")
{
	...
	$profile_page = "index.php?module=cloudflare-recent_visitors&amp;onlyshow=all";
