...
$dev_request = cloudflare_dev_mode($cache->read('cloudflare_zone_id'));
...
if(!$mybb->input['action'])
{
	...
	if($in_dev)
	{
		echo "<div class=\"success\">CloudFlare is currently in development mode. This will expire in ". gmdate("H:i:s", $dev_request->result->time_remaining) . "</div>";
	}
	else
	{
		echo '<div id="inner">
...
elseif($mybb->input['action'] == "change")
{
	...
	$page->output_header("CloudFlare Manager - Development Mode");
	$request = cloudflare_dev_mode($cache->read('cloudflare_zone_id'), true);
